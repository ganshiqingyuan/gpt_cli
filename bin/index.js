...
import { genereteNestMModule } from './tools/nest_module.js';
...
if (template === 'nest_module') {
    genereteNestMModule(dirName, __dirname);
}
else {
    const spinners = [ora('创建模块中...')];
    spinners[0].start();
    fs.mkdirSync(dirName);
    await fse.copy(path.resolve(__dirname, `../templates/${template}`), `${dirName}`, (err) => {
        if (err) {
            console.error(err);
