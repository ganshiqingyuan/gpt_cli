...
export async function genereteNestMModule(moduleName, entryPath) {
    const spinners = [ora('module生成中...')];
    spinners[0].start();
    fse.copy(path.resolve(entryPath, `../templates/admin`), `${moduleName}`, (err) => {
        if (err) {
            console.error(err);
...
function renameFilesRecursive(dir, from, to) {
    fs.readdirSync(dir).forEach((it) => {
        ...
        if (itsStat.isFile()) {
            ...
            fs.writeFileSync(itsPath, template({
                name: to,
                uperName: to.substring(0, 1).toUpperCase() + to.substring(1),
                repositoryName: to,
            }));
        }
